 ...
 */
public abstract class CsvReader implements Iterable<List<byte[]>> {
    protected Logger log = LoggerFactory.getLogger(getClass());

    protected CsvReaderConfig config;
    /** 可以操作 buffer.array() 的 buffer，尝试过 mapped，性能提升不理想就枪毙了 */
    ...
    private void noTextQualifierParse() throws IOException {
        ...
        while (hasMoreData && startedColumn) {
            ...
            // 读取到了列分隔符，标识一列的结束
            if (currentLetter == config.getDelimiter()) {
                endColumn();
            }
    ...
    private void textQualifierParse() throws IOException {
        ...
        // 上一个字符可能双引号的第一个引号，也有可能是文本边界，也有可能是协议中的转义符号
...
//                        System.out.println("xxx");
    ...
    private void endColumn() {
        // 获取这一列的数据，需要分情况
        // 当一列的数据在 buffer 中都存在的时候，可以直接把一列数据都获取出来，不需要中转
        // 所以当 列中转 存储为 0 的时候，就符合这一设定
        if (columnBuffer.position() == 0) {
            int position = buffer.position();
            // position - 1 跳过分隔符
            byte[] columnValue = new byte[position - 1 - startedColumnPosition];
            ...
            // 从列 buffer 中读取数据
            byte[] columnValue = new byte[columnBuffer.position() - 1];
...
//                columnBuffer.put(bytes);
                System.arraycopy(buffer.array(), startedColumnPosition, columnBuffer.array(), columnBuffer.position(), length);
                columnBuffer.position(columnBuffer.position() + length);
            }
        }
        // 数据 copy 之后需要更新列开始在 buffer 中的位置
        // 在 checkDataLength 中，当本方法结束的时候，startedColumnPosition 就会被重置为 0
        // 这里主要用于文本边界模式：跳过一些需要丢弃的符号，如跳过双引号
    ...
    class CsvIterator implements Iterator<List<byte[]>> {

        @Override
        public boolean hasNext() {
            try {
                return CsvReader.this.hashNext();
            } catch (IOException e) {
